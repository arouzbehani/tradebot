    ...
    def calculate_dict(self):
        if self.analysis == '1.0':
            for tf in self.tfs:
                ...
                df_long = df[-self.trend_limit_long:].reset_index(drop=True)
                df_short = df[-self.trend_limit_short:].reset_index(drop=True)
                df_long, _, _, _, _, _, _, _  = pivot_helper.find_pivots(
                    df_long, self.long_term_pivot_candles, self.long_term_pivot_candles, self.waves_number, short=False)
