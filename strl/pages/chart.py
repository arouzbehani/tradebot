...
def DrawChart(
    ...
    entry_signals=True,
    exit_signals=False,
    entry_signal_mode="All",
    left_candles=3,
    right_candles=3,
    waves_number=3,
    resistance_trend=False,
    support_trend=False,
    npiv=3,
):
    # symbol = 'BTC_USDT'
    ...
    if symbol != None and tf != None and exch != None:
        ...
        if df.size:
            ...
            if support_trend:
                ...
                if npiv == 0:
                    m = len(down_ys)
                r2_down, trend_down_xs, trend_down_ys = helper.ReturnTrend_From_Comb(
                    helper.Return_Combo(
                        xs=down_xs.values, ys=down_ys.values, n=m, r_min=0.92
                    ),
                    bounds=boudry_xs,
                )
            ...
            if resistance_trend:
                ...
                sorted_resist_combs = sorted(
                    combs, key=lambda c: helper.r2(c), reverse=True
                )

                r2_up, trend_up_xs, trend_up_ys = helper.ReturnTrend_From_Comb(
                    helper.Return_Combo(
                        xs=up_xs.values, ys=up_ys.values, n=m, r_min=0.92
                    ),
                    bounds=boudry_xs,
                )
            ...
            if patterns_res:
                ...
                fig.add_trace(fig_bull, row=rownum, col=1)
                rownum += 1
                fig.add_trace(fig_bear, row=rownum, col=1)
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(
                            ...
                            line=dict(width=2, color=3),
                        ),
                        name="adx entry signal",
                    )
                )
                ...
                if entry_signals:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                size=8, symbol="arrow-wide", line=dict(width=2, color=2)
                            ),
                            name="fi entry signal",
                        )
                    )
                ...
                if entry_signals:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                size=8, symbol="circle", line=dict(width=2, color=1.5)
                            ),
                            name="bb entry signal",
                        ),
                        row=1,
                        col=1,
                    )
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                size=8, symbol="circle-x", line=dict(width=2, color=1.5)
                            ),
                            name="bb squeez entry signal",
                        ),
                        row=1,
                        col=1,
                    )
            if False:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="markers",
                        marker=dict(
                            size=3, symbol="triangle-up", line=dict(width=2, color=2.5)
                        ),
                        name="break out",
                    ),
                    row=1,
                ...
                if entry_signals:
                    ...
                    if dvg_res:
                        fig.add_trace(
                            go.Scatter(
                                x=px,
                                y=py,
                                name="divergence on graph",
                                line=dict(width=3, color="#ff6c00"),
                            ),
                            row=1,
                            col=1,
                        )
                        fig.add_trace(
                            go.Scatter(
                                x=rsi_x,
                                y=rsi_y,
                                name="divergence on rsi",
                                line=dict(width=2, color="#ff6c00"),
                            ),
                            row=rownum,
                            col=1,
                        )
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers",
                            marker=dict(
                                size=8, symbol="diamond", line=dict(width=2, color=1.5)
                            ),
                            name="rsi entry signal",
                        ),
                        row=1,
                        col=1,
                    )
...
def DrawTheChart():
    DrawChart(
        ...
        read_ema=ema_check,
        read_sma=sma_check,
        read_ichi=ichi_check,
        read_patterns=patterns_check,
        read_rsi=rsi_check,
        entry_signals=entry_signals_check,
        entry_signal_mode=entry_signal_mode,
        left_candles=pivot_candle_left,
        right_candles=pivot_candle_right,
...
with st.sidebar:
    ...
    pivot_candle_right = st.number_input("Right Candles:", min_value=2, value=6)
...
with st.container():
    ...
    if cols[len(tfs)].button("Download Latest"):
        if exch == "Kucoin":
            ...
            mr.ReadYahooMarket(
                timeframes=tfs, local=local, testdata=False, symbol=symbol
            )
        elif exch == "Forex":
            mr.ReadForexMarket(
                timeframes=tfs, local=local, testdata=False, symbol=symbol
