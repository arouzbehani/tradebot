...
def DrawChart(limit=500, read_patterns=False, read_rsi=False, read_bb=True, read_sma=True, read_ema=False, read_vol=True,read_fi=False, chart_height=800,
    ...
    if (q.__contains__('symbol') and q.__contains__('tf') and q.__contains__('exch')):
        ...
        if (df.size):
            ...
            df,up_points,down_points,sidepoints,power_ups,power_downs,power_weaking_ups,power_weaking_downs=pivot_helper.find_pivots(df,left_candles,right_candles,waves_number)
            ...
            pointpos_df=pd.DataFrame(data=df[~pd.isnull(df['pointpos'])],columns=['time','pointpos'])
            #st.dataframe(df)
            fig.add_trace(
                go.Candlestick(x=df['time'], open=df['open'], close=df['close'], high=df['high'], low=df['low'], name=symbol.replace('_', '/')), row=1, col=1
            )
            fig.add_trace(
                go.Scatter(x=pointpos_df['time'], y= pointpos_df['pointpos'],line=dict(color="#3d5ab2"), name='wave line')
            )            
            fig.add_trace(
                go.Scatter(x=df['time'], y=power_ups, mode='markers', marker=dict(
                    size=10, color="green" , symbol='star'), name='stronging up')
            )
            fig.add_trace(
                go.Scatter(x=df['time'], y=power_downs, mode='markers', marker=dict(
                    size=10, color="red" , symbol='star'), name='stronging down')
            )           
            fig.add_trace(
                go.Scatter(x=df['time'], y=power_weaking_ups, mode='markers', marker=dict(
                    size=10, color="green" , symbol='circle-open'), name='weaking up')
            )
            fig.add_trace(
                go.Scatter(x=df['time'], y=power_weaking_downs, mode='markers', marker=dict(
                    size=10, color="red" , symbol='circle-open'), name='weaking down')
            )               
            rownum = 1
            if (patterns_res):
                rownum += 1
...
with st.sidebar:
    limit = st.number_input("Data Limit:", min_value=100, value=500)
    st.write("Pivot Settings:")
    pivot_candle_left=st.number_input("Left Camndles:",min_value=2,value=3)
    pivot_candle_right=st.number_input("Right Camndles:",min_value=2,value=3)
    ...
    if(ML_check):
        forecast_out=st.number_input("Forecast Number:",min_value=5,max_value=30,value=20)
